...
\newcommand{\figcolvertw}{0.76\linewidth}
\newcommand{\figcolverth}{0.56\textheight}
% Fig.~1 (full-width star figure); Fig.~4 and Fig.~5 use a slightly smaller double-column width.
\newcommand{\figdbl}{0.98\textwidth}
\newcommand{\figdblsmall}{0.86\textwidth}

\begin{document}

\title{Baseline-Validated Receiver Framework for OAM-Multiplexed Optical Wireless Links Under Atmospheric Turbulence}
...
\thanks{S.~Davuluri, B.~P, A.~Bhowmick, and S.~Arya are with [Institution], [City], [Country] (e-mail: corresponding.author@institution.edu). Replace bracketed placeholders and ORCID entries before IEEE TWC submission.}%
}
...
\begin{tabularx}{\textwidth}{lXXcXX}
\toprule
\textbf{Method} & \textbf{Task} & \textbf{Input} & \textbf{Pilot} & \textbf{Scope} & \textbf{Limitation} \\
\midrule
MMSE~\cite{huang2014100} & Symbol recovery & Coherent modal outputs & Pilot-aided & Weak to moderate turbulence & Ill-conditioned inversion under strong crosstalk \\
K-best~\cite{ge2021oam} & Symbol recovery & Coherent modal outputs & Pilot-aided & Weak to moderate turbulence & Higher complexity, tied to channel inversion \\
CNN (Mode)~\cite{doster2017machine} & Mode classification / demux & Intensity images & Pilot-free & Weak to moderate & No direct symbol recovery \\
End-to-end DL~\cite{karanov2018endtoend} & Fiber IM/DD transceiver (NN) & Learned waveforms / intensity & None (joint) & Lab + simulation fiber link & Fiber channel; not FSO / OAM turbulence \\
AlexNet~\cite{zhao2020cnn} & Turbulence compensation & Intensity images & Auxiliary & Moderate turbulence & Not end-to-end demodulation \\
\midrule
\textbf{This work} & Baseline validation + neural evaluation & Coherent baseline; intensity-only neural & Pilot-aided; pilot-free (neural) & Validated across five $C_n^2$ regimes, ideal to extreme & Intensity-only neural degrades under strong/extreme turbulence; phase information not exploited \\
\bottomrule
\end{tabularx}
\end{table*}
...
Acronyms are listed in Table~\ref{tab:abbreviations}. What follows develops the OAM optical link model and the classical receiver built on top of it. Fig.~\ref{fig:comprehensive_system_model} summarizes the shared front end: coded QPSK symbols excite multiplexed LG modes, the field propagates through Kolmogorov turbulence with a swept $C_n^2$, and the observation is split into a coherent branch (modal projection, pilot-aided $\hat{\mathbf{H}}$, MMSE, LDPC) and an intensity-only neural branch evaluated on matched realizations. Fig.~\ref{fig:system_architecture} details the classical signal flow from encoding through decoding.

\begin{figure*}[!t]
\centering
\includegraphics[width=\figdbl]{images/system_model_diagram.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\figcolvertw,height=\figcolverth,keepaspectratio]{images/neural/system_architecture.png}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\figdbl]{images/neural/fso_turbulence_prop.png}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\figdblsmall]{images/neural/curriculum_training.png}
...
\subfloat[Ideal.\label{fig:ch_ideal}]{%
  \includegraphics[width=0.31\textwidth]{images/classical/ideal_channel_matrix.png}%
} &
\subfloat[Weak.\label{fig:ch_weak}]{%
  \includegraphics[width=0.31\textwidth]{images/classical/weak_channel_matrix.png}%
} &
\subfloat[Moderate.\label{fig:ch_moderate}]{%
  \includegraphics[width=0.31\textwidth]{images/classical/moderate_channel_matrix.png}%
}
\end{tabular}\\[0.8ex]
\begin{tabular}{@{}c@{\hspace{6pt}}c@{}}
\subfloat[Transition/strong.\label{fig:ch_transition}]{%
  \includegraphics[width=0.31\textwidth]{images/classical/transition_channel_matrix.png}%
} &
\subfloat[Extreme.\label{fig:ch_extreme}]{%
  \includegraphics[width=0.31\textwidth]{images/classical/extreme_channel_matrix.png}%
}
\end{tabular}
...
Three points are worth emphasizing. LDPC gain is substantial only in weak turbulence; once $C_n^2$ passes roughly $10^{-15}$, the pre- and post-decoding curves merge, which suggests the soft information is too corrupted to recover. The rise in ill-conditioning, with $\kappa(\hat{\mathbf{H}})$ moving from near unity to beyond 17, tracks the BER increase closely and is consistent with MMSE noise amplification as the channel matrix degrades. Beyond $10^{-14}$, the matrices lose clear diagonal structure and BER settles near 0.5, making recovery effectively infeasible for this receiver design. Table~\ref{tab:performance_summary} summarizes this regime behavior.
...
\subfloat[Ideal.\label{fig:neural_const_ideal}]{%
  \includegraphics[width=0.31\textwidth]{images/neural/results/cnn_constellation_convnext_tiny_curriculum_lvl1_ideal.png}%
} &
\subfloat[Weak.\label{fig:neural_const_weak}]{%
  \includegraphics[width=0.31\textwidth]{images/neural/results/cnn_constellation_convnext_tiny_curriculum_lvl2_weak.png}%
} &
\subfloat[Moderate.\label{fig:neural_const_moderate}]{%
  \includegraphics[width=0.31\textwidth]{images/neural/results/cnn_constellation_convnext_tiny_curriculum_lvl3_moderate.png}%
}
\end{tabular}\\[0.8ex]
\begin{tabular}{@{}c@{\hspace{6pt}}c@{}}
\subfloat[Strong.\label{fig:neural_const_strong}]{%
  \includegraphics[width=0.31\textwidth]{images/neural/results/cnn_constellation_convnext_tiny_curriculum_lvl4_strong.png}%
} &
\subfloat[Extreme.\label{fig:neural_const_extreme}]{%
  \includegraphics[width=0.31\textwidth]{images/neural/results/cnn_constellation_convnext_tiny_curriculum_lvl5_extreme.png}%
}
\end{tabular}
