...
class SimplifiedLDPC:
    ...
    def decode_simple(self, received_bits):
        """
        "Cheating" decoder for a perfect channel.
        Since this is a systematic code c = [u | p], and we assume
        the channel is perfect (or corrected by the equalizer),
        we just extract the information bits 'u'.
        """
        num_blocks_full = len(received_bits) // self.n
        remaining_bits = len(received_bits) % self.n
        
        decoded_bits_list = []

        # 1. Decode all the full codewords
        if num_blocks_full > 0:
            trunc_len = num_blocks_full * self.n
            full_blocks = received_bits[:trunc_len]
            
            # Reshape into blocks and extract info bits
            blocks = full_blocks.reshape((num_blocks_full, self.n))
            info_blocks = blocks[:, :self.k] # Get first k bits of all blocks
            decoded_bits_list.append(info_blocks.flatten())

        # 2. Handle the final, partial codeword
        if remaining_bits > 0:
            # We have a partial block, e.g., 1016 bits
            # Just extract the info bits from it
            partial_block = received_bits[num_blocks_full * self.n:]
            
            # Extract the info bits, but don't read past the end
            info_bits_in_partial = min(self.k, len(partial_block))
            decoded_bits_list.append(partial_block[:info_bits_in_partial])

        if not decoded_bits_list:
            return np.array([], dtype=int)
            
        return np.concatenate(decoded_bits_list).astype(int)
