...
class FSORx:
    ...
    def receive_frame(self, rx_field_sequence, tx_frame: FSO_MDM_Frame, original_data_bits: np.ndarray, verbose: bool = True, bypass_ldpc: bool = False):
        ...
        if not use_mmse:
            # ZF with regularization for stability
            try:
            ...
            # MMSE with proper regularization
            sigma2 = max(noise_var, 1e-12)
            # Add regularization based on condition number
            reg_factor = max(1e-6, sigma2 * (1 + cond_H / 1e4))
            try:
                W_mmse = inv(H.conj().T @ H + reg_factor * np.eye(self.n_modes)) @ H.conj().T
                S_est = W_mmse @ Y_data
            except np.linalg.LinAlgError:
